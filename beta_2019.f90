...
INTEGER, PARAMETER :: error_unit          = 15   ! error file (.err)
INTEGER, PARAMETER :: extra_results_unit  = 16   ! extra results file (.xrs) (for printing additional results)
...
TYPE particle_data
  ...
  REAL(KIND=real8)                 :: mass, radius, inertia, charge, specific_heat
  ...
  REAL(KIND=real8)                 :: initial_temperature, temperature
  REAL(KIND=real8), DIMENSION(3)   :: xyz_coordinates, initial_velocity
  REAL(KIND=real8), DIMENSION(3)   :: conduction
...
TYPE material_set_data
  ...
  REAL(KIND=real8) :: specific_heat, thermal_conductivity
  REAL(KIND=real8) :: drag_heating_efficiency, radiative_efficiency
...
TYPE wall_data
  ...
  REAL(KIND=real8)   :: rolling_resistance_coeff, rolling_resistance_damping_ratio
  REAL(KIND=real8)	 :: initial_temperature, temperature
  REAL(KIND=real8)   :: thermal_conductivity, specific_heat
...
TYPE general_control_variables
  ...
  CHARACTER(LEN=3)   :: rotational_dofs, temperature_dofs, temperature_wall_dofs, periodic_bc
  CHARACTER(LEN=3)   :: temperature_side_one_wall_dofs, temperature_side_two_wall_dofs, temperature_side_three_wall_dofs, temperature_side_four_wall_dofs, &
						& temperature_side_five_wall_dofs
...
TYPE system_data
  ...
  INTEGER(KIND=int4) :: critical_temperature
  INTEGER(KIND=int4) :: no_particles_with_initial_given_forces_and_moments 
  INTEGER(KIND=int4) :: no_rigid_walls, no_springs, no_pressure_surfaces 
...
! add gravity field contribution to environment force vector
fenv = fenv + mi*g  !+ qi*E + qi*(vi .vector. B)
...
! add pressure gradient contribution to environment force vector (pressure gradient = pressure drop per unit length; only x-direction is considered here)
...
SUBROUTINE compute_thermomechanical_sphere_environment_forces(particle_number,R,V,system,fenv,menv)
...
!xxxxxxxxxxxx modification in vf for Tarek´s fragments example xxxxxxxxxxxx
!vf = 0.0_real8
...
SUBROUTINE compute_thermomechanical_sphere_environment_heat_transfer(particle_number,R,V,T,system,htenv)

! dummy arguments
INTEGER(KIND=int4), INTENT(IN) :: particle_number
TYPE(array_of_vectors), DIMENSION(:), POINTER :: R, V, T
TYPE(system_data) :: system
REAL(KIND=real8)  :: htenv
...
! add convective contribution to environment heat transfer
htenv = hi*Ais*(tenv-ti)

! add drag-induced heating contribution to environment heat transfer
htenv = htenv + gammai*DOT_PRODUCT(fdrag,vfminusvi)

! add radiative contribution to environment heat transfer
htenv = htenv + epsiloni*St_Boltz*Ais*(tenv**4 - ti**4)

END SUBROUTINE compute_thermomechanical_sphere_environment_heat_transfer
!!---------------------------------------------------------------------------------------------------------------------
...
SUBROUTINE compute_particle_environment_forces(particle_number,particle_kind,R,V,system,fenv,menv)
...
SELECT CASE (particle_kind)
  ...
  CASE ("thermomechanical_sphere")
    CALL compute_thermomechanical_sphere_environment_forces(particle_number,R,V,system,fenv,menv)
...
SUBROUTINE compute_particle_environment_heat_transfer(particle_number,particle_kind,R,V,T,system,htenv)
...
SELECT CASE (particle_kind)
  ...
  CASE ("thermomechanical_sphere")
    CALL compute_thermomechanical_sphere_environment_heat_transfer(particle_number,R,V,T,system,htenv)
...
END SUBROUTINE compute_particle_environment_heat_transfer
!!---------------------------------------------------------------------------------------------------------------------
...
! get grid data
lx = system%grid%xend - system%grid%xbeg ! dimensões do dominio
ly = system%grid%yend - system%grid%ybeg ! dimensões do dominio
lz = system%grid%zend - system%grid%zbeg ! dimensões do dominio
ndivx = system%grid%ndivx ! número de divisões, subdomínios ou células
ndivy = system%grid%ndivy ! número de divisões, subdomínios ou células
ndivz = system%grid%ndivz ! número de divisões, subdomínios ou células
...
! allocate grid cells
ALLOCATE(system%grid%cell(0:(ndivx+1),0:(ndivy+1),0:(ndivz+1))) ! “endereço” de um subdomínio ou célula

!initialize cell lists
DO ix=0,ndivx+1
  DO iy=0,ndivy+1
    DO iz=0,ndivz+1
      system%grid%cell(ix,iy,iz)%no_particles_into_cell = 0 ! Construção das listas de partículas contidas nos subdomínios ou células
...
DO i=1,system%no_particles
  ...
  system%particle(i)%cell_address(1) = cellx ! endereço da célula em que uma partícula i se encontra nesse instante
  system%particle(i)%cell_address(2) = celly ! endereço da célula em que uma partícula i se encontra nesse instante
  system%particle(i)%cell_address(3) = cellz ! endereço da célula em que uma partícula i se encontra nesse instante
...
DO ix=1,ndivx
  DO iy=1,ndivy
    DO iz=1,ndivz
      ...
      DO pi=1,cell_no_particles ! NÚMERO DE PARTÍCULAS DENTRO DE LA CÉLULA
        i=system%grid%cell(ix,iy,iz)%particle_list(pi) ! CONTADOR DE LA PARTÍCULA DENTRO DE LA CÉLULA 
        DO iix=ix-1,ix+1
          DO iiy=iy-1,iy+1
            DO iiz=iz-1,iz+1
...
iaux_list(1:(ncpi-1)) = system%particle(i)%contacting_particles    ! Se resta porque cuando j vale su mayor valor, suma uno demás
...
SELECT CASE (system%solution_cv%rolling_resistance_model)
  ...
  CASE ("avci_torque")
    CALL sphere_sphere_avci_torque_rolling_resistance(i,j,wi,wj,overlap,nij,norm_fcon,vrelctang,system,mirol)
  CASE DEFAULT
...
SELECT CASE (system%solution_cv%rolling_resistance_model)
  ...
  CASE ("avci_torque")
    CALL sphere_wall_avci_torque_rolling_resistance(i,k,wi,ww,overlap,nout,norm_fcon,vrelctang,system,mirol)
  CASE DEFAULT
    WRITE(error_unit,*) " Rolling resistance model not recognized or not yet implemented"
    STOP
END SELECT
...
SUBROUTINE sphere_sphere_avci_torque_rolling_resistance(i,j,wi,wj,overlap,nij,norm_fcon,vrelctang,system,mrol)

! dymmy arguments
TYPE(system_data)  :: system
INTEGER(KIND=int4) :: i, j
REAL(KIND=real8)   :: overlap, norm_fcon
REAL(KIND=real8), DIMENSION(3) :: wi, wj, vrelctang, nij
REAL(KIND=real8), DIMENSION(3), INTENT(OUT) :: mrol
...
REAL(KIND=real8)   :: Ri, mi, ji, Rj, mj, jj, Req, Ei, Ej, ni_i, ni_j, miri, mirj, mir, xsiri, xsirj, xsir, dt, &
                      Gi, Gj, Geq, norm_wrel, kr, jr, dr, aux_ji, aux_jj, norm_mrol_trial    
REAL(KIND=real8), DIMENSION(3) :: wrel, sij_trial, sij, dxij_trial, xijr, xij_trial, xij, mrol_trial, vrol, phi_dot
...
! compute rolling translational velocity
vrol = -Req*((wrel .vector. (-nij)) + half*(one/Ri-one/Rj)*vrelctang)

! compute rolling angular velocity
phi_dot = (one/Req)*(vrol .vector. (-nij))

! compute trial incremental elongation of rolling resistance spring
dxij_trial = phi_dot*dt
...
! compute trial rolling resistance moment
mrol_trial = - kr*xij_trial - dr*phi_dot
...
! check for stick or slip (rolling resistance yield criterion: verify if trial rolling resistance moment violates the maximum possible value)
IF (norm_mrol_trial<=(mir*Req*norm_fcon)) THEN
  mrol = mrol_trial
  xij = xij_trial
  sij = sij_trial
...
END SUBROUTINE sphere_sphere_avci_torque_rolling_resistance
!!---------------------------------------------------------------------------------------------------------------------


SUBROUTINE sphere_wall_avci_torque_rolling_resistance(i,k,wi,ww,overlap,nout,norm_fcon,vrelctang,system,mrol)

! dymmy arguments
TYPE(system_data)  :: system
INTEGER(KIND=int4) :: i, k
REAL(KIND=real8)   :: overlap, norm_fcon
REAL(KIND=real8), DIMENSION(3) :: wi, ww, vrelctang, nout
REAL(KIND=real8), DIMENSION(3), INTENT(OUT) :: mrol
...
REAL(KIND=real8)   :: Ri, mi, ji, Req, Ei, ni_i, miri, xsiri, mirw, xsirw, Gi, Geq, mir, xsir, &
                      dt, kr, norm_wrel, jr, dr, norm_mrol_trial
REAL(KIND=real8), DIMENSION(3) :: wrel, sij_trial, sij, dxiw_trial, xiwr, xiw_trial, xiw, mrol_trial, vrol, phi_dot
...
! compute rolling resistance stiffness
kr = Req*norm_fcon
...
! compute rolling translational velocity
vrol = -Req*((wrel .vector. (-nout)) + half*(one/Ri)*vrelctang)

! compute rolling angular velocity
phi_dot = (one/Req)*(vrol .vector. (-nout))

! compute trial incremental elongation of rolling resistance spring
dxiw_trial = phi_dot*dt
...
! compute trial rolling resistance moment
mrol_trial = - kr*xiw_trial - dr*phi_dot
...
END SUBROUTINE sphere_wall_avci_torque_rolling_resistance
!!---------------------------------------------------------------------------------------------------------------------
...
SUBROUTINE sphere_sphere_contact_by_hertz_with_stickslip(i,j,overlap,norm_ri_m_rj,xi,xj,ti,tj,vi,vj,wi,wj,nij,system,htcond,a_1,l_1,E_i,E_j,h_i_j) !aqui_stick

! dymmy arguments
TYPE(system_data)  :: system
INTEGER(KIND=int4) :: i, j
REAL(KIND=real8)   :: overlap, ti, tj, norm_ri_m_rj, htcond, a_1, l_1, E_i, E_j, h_i_j
REAL(KIND=real8), DIMENSION(3) :: vi, vj, wi, wj, nij, xi, xj


! local variables
INTEGER(KIND=int4) :: s, imat, jmat, cp, jaddr, iaddr, ss, T_crit
REAL(KIND=real8)   :: Ei, Eii, Ej, Ejj, ni_i, ni_j, Gi, Gj, ri, rj, mi, mj, Eeq, Geq, req, meq, deltat, xsini, xsinj, xsin, &
                      xsiti, xsitj, xsit, misi, misj, mis, midi, midj, mid, kn, dn, vin, vjn, vreln, &
                      norm_fcon, kt, dt, norm_ffri_trial, ki, kj, kij, a_i_j, contact_area, Li, Ei_updated, Ej_updated
...
imat = system%particle(i)%material_set_number
jmat = system%particle(j)%material_set_number
T_crit = system%critical_temperature
...
IF (system%general_cv%temperature_dofs=="off") THEN
	Kij	 = 0.0_real8
END IF
kij	 = ((ri+rj)*(ki*kj)) / ((ki*rj)+(kj*ri))

! update elasticity modulus
Ei_updated = Ei * (EXP(1 - (ti/T_crit)))
Ej_updated = Ej * (EXP(1 - (tj/T_crit)))

Eii = MIN(Ei,Ei_updated)
Ejj = MIN(Ej,Ej_updated)

IF (system%general_cv%temperature_dofs=="off") THEN
	Eii = system%material_set(imat)%elasticity_modulus
	Ejj = system%material_set(jmat)%elasticity_modulus
END IF

E_i = MIN(Ei,Ei_updated)
E_j = MIN(Ej,Ej_updated)

IF (system%general_cv%temperature_dofs=="off") THEN
	E_i = system%material_set(imat)%elasticity_modulus
	E_j = system%material_set(jmat)%elasticity_modulus
END IF
...
!Gi = Ei/(2.0_real8*(one+ni_i))
!Gj = Ej/(2.0_real8*(one+ni_j))
Gi = Eii/(2.0_real8*(one+ni_i))
Gj = Ejj/(2.0_real8*(one+ni_j))

! compute equivalent properties for the contacting pair
!Eeq = Ei*Ej/(Ei + Ej - Ei*ni_j*ni_j - Ej*ni_i*ni_i)
Eeq = Eii*Ejj/(Eii + Ejj - Eii*ni_j*ni_j - Ejj*ni_i*ni_i)
Geq = Gi*Gj/(two*Gi + two*Gj - Gi*ni_j - Gj*ni_i)
req = ri*rj/(ri+rj)
meq = mi*mj/(mi+mj)
...
! compute thermal conduction heat transfer
Li = 0.5_real8*(norm_ri_m_rj - ((rj**2 - ri**2)/(norm_ri_m_rj))) 
!a_i_j = SQRT(ri**2 - Li**2)


!=================================================================
l_1 = 0.5_real8*(norm_ri_m_rj - ((ri**2 - rj**2)/(norm_ri_m_rj)))
!=================================================================

!a_i_j = SQRT(req * overlap)
!contact_area = pi * (a_i_j**2)
...
htcond = kij * (((tj - ti) / norm_ri_m_rj) * contact_area)

!h_i_j = htcond / (tj - ti)

! update particles´ total tempetarures (conduction)
20	system%particle(i)%conduction = system%particle(i)%conduction + htcond
	system%particle(j)%conduction = system%particle(j)%conduction - htcond

END SUBROUTINE sphere_sphere_contact_by_hertz_with_stickslip
!!---------------------------------------------------------------------------------------------------------------------


SUBROUTINE sphere_wall_contact_by_hertz_with_stickslip(i,k,overlap,overlap_w,a_w,ti,tk,diw,vi,vw,wi,ww,nout,system)

! dymmy arguments
TYPE(system_data)  :: system
INTEGER(KIND=int4) :: i, k
REAL(KIND=real8)   :: overlap, ti, tk, diw, overlap_w, a_w
REAL(KIND=real8), DIMENSION(3) :: vi, vw, wi, ww, nout

! local variables
INTEGER(KIND=int4) :: s, imat, waddr, j
REAL(KIND=real8)   :: Ei, Eii, ni_i, Gi, ri, mi, Eeq, Geq, req, meq, misi, misw, mis, midi, midw, mid, deltat, xsini, &
                      xsinw, xsin, xsiti, xsitw, xsit, vin, vwn, vreln, kn, dn, norm_fcon, kt, dt, norm_ffri_trial, &
					  ki, kw, kiw, ht_wcond, a_i_j, contact_area, Ei_updated, T_crit, ro_1, a_ii
...
! compute thermal wall conduction heat transfer

	IF (system%general_cv%temperature_side_one_wall_dofs=="off".and.system%general_cv%temperature_side_two_wall_dofs=="off".and.system%general_cv%temperature_side_three_wall_dofs=="off") THEN
		GO TO 20	 		
	END IF
		...
		!a_i_j = SQRT(req*overlap)
		!contact_area = pi * (a_i_j**2)
		contact_area = pi * (ri**2 - diw**2)
		a_w = contact_area

		ht_wcond = kiw * (((tk - ti) / diw) * contact_area) !aqui

		! update walls´ total tempetarures (conduction)
		system%particle(i)%conduction = system%particle(i)%conduction + ht_wcond  	

20 END SUBROUTINE sphere_wall_contact_by_hertz_with_stickslip
!!---------------------------------------------------------------------------------------------------------------------
...
MODULE hertz_contact_with_adhesion_and_stsl_friction
...
SUBROUTINE sph_sph_cont_by_hertz_w_adh_and_ss(i,j,overlap,vi,vj,wi,wj,nij,system)
...
END SUBROUTINE sph_sph_cont_by_hertz_w_adh_and_ss
!!---------------------------------------------------------------------------------------------------------------------


SUBROUTINE sph_wall_cont_by_hertz_w_adh_and_ss(i,k,overlap,vi,vw,wi,ww,nout,system)
...
END SUBROUTINE sph_wall_cont_by_hertz_w_adh_and_ss
!!---------------------------------------------------------------------------------------------------------------------


END MODULE hertz_contact_with_adhesion_and_stsl_friction
...
USE hertz_contact_with_adhesion_and_stsl_friction
...
SUBROUTINE resolve_sphere_sphere_contact(i,j,overlap,norm_ri_m_rj,xi,xj,ti,tj,vi,vj,wi,wj,aidelta,ajdelta,nij,system,htcond,a_1,l_1,E_i,E_j,h_i_j)

! dymmy arguments
TYPE(system_data)  :: system
INTEGER(KIND=int4) :: i, j
REAL(KIND=real8)   :: overlap, ti, tj, norm_ri_m_rj, htcond, a_1, l_1, E_i, E_j, h_i_j
REAL(KIND=real8), DIMENSION(3) :: vi, vj, wi, wj, aidelta, ajdelta, nij, xi, xj
...
SELECT CASE (system%solution_cv%contact_model)
  ...
  CASE ("hertz_with_stickslip_friction")
    CALL sphere_sphere_contact_by_hertz_with_stickslip(i,j,overlap,norm_ri_m_rj,xi,xj,ti,tj,vi,vj,wi,wj,nij,system,htcond,a_1,l_1,E_i,E_j,h_i_j)
  ...
  CASE ("hertz_with_adhesion_and_stickslip_friction")
    CALL sph_sph_cont_by_hertz_w_adh_and_ss(i,j,overlap,vi,vj,wi,wj,nij,system)
  CASE DEFAULT
...
SUBROUTINE resolve_sphere_wall_contact(i,k,overlap,overlap_w,a_w,ti,tk,diw,vi,vw,wi,ww,ri,aidelta,n_out,system)
...
SELECT CASE (system%solution_cv%contact_model)
  ...
  CASE ("hertz_with_stickslip_friction")
    CALL sphere_wall_contact_by_hertz_with_stickslip(i,k,overlap,overlap_w,a_w,ti,tk,diw,vi,vw,wi,ww,n_out,system)
  CASE ("hertz_with_thornton_friction")
  ...
  CASE ("hertz_with_adhesion_and_stickslip_friction")
    CALL sph_wall_cont_by_hertz_w_adh_and_ss(i,k,overlap,vi,vw,wi,ww,n_out,system)
  CASE DEFAULT
    WRITE(error_unit,*) " Contact model not recognized or not yet implemented"
    STOP
END SELECT
...
SUBROUTINE contact_solver_for_spheres(i,R,V,Tt,Tw,A,W,system,htcond,overlap_1,overlap_w,a_w,a_1,l_1,E_i,E_j,h_i_j)

! dymmy arguments
INTEGER(KIND=int4) :: i
TYPE(array_of_vectors), DIMENSION(:), POINTER :: R, V, Tt, Tw
TYPE(array_of_vectors), DIMENSION(:), POINTER, OPTIONAL :: A, W
TYPE(system_data)  :: system
REAL(KIND=real8)   :: htcond, overlap_1, a_1, l_1, E_i, E_j, overlap_w, a_w, h_i_j
...
SELECT CASE (system%solution_cv%contact_detection_algorithm)
  ...
  CASE ("loop_over_all_particles")
    IF (system%solution_cv%contact_model=="impulse_based_contact") THEN
      CALL detect_and_resolve_sphere_contacts_by_loap_for_hard_spheres(i,R,V,A,W,system)
    ELSE
      CALL detect_and_resolve_sphere_contacts_by_looping_over_all_part(i,R,V,A,W,system)
    END IF

  CASE ("verlet_list")
    IF (system%solution_cv%contact_model=="impulse_based_contact") THEN
      WRITE(error_unit,*) " Contact detection algorithm not yet implemented for the selected contact model"
      STOP
    ELSE
      CALL detect_and_resolve_sphere_contacts_by_verlet_lists(i,R,V,Tt,Tw,A,W,system,htcond,overlap_1,overlap_w,a_w,a_1,l_1,E_i,E_j,h_i_j)
    END IF
  ...
  CASE ("binning")
    IF (system%solution_cv%contact_model=="impulse_based_contact") THEN
      ...
      STOP
    ELSE
      CALL detect_and_resolve_sphere_contacts_by_binning(i,R,V,A,W,system)
    END IF
...
SUBROUTINE detect_and_resolve_sphere_contacts_by_loap_for_hard_spheres(i,R,V,A,W,system)
...
TYPE(array_of_vectors), DIMENSION(:), POINTER, OPTIONAL :: A, W
TYPE(system_data)  :: system
REAL(KIND=real8)   :: ti, tj, tk, htcond, a_1, l_1, E_i, E_j, overlap_w, a_w, h_i_j

! local variables
INTEGER(KIND=int4) :: j, s, k
REAL(KIND=real8)   :: norm_ri_m_rj, overlap, vin, vjn, normal_relative_vel, dt, diw, vwn
REAL(KIND=real8), DIMENSION(3) :: ri, rj, ri_m_rj, nij, vi, vj, wi_tpdt, wj_tpdt, r1, ri1, n_out, vw, ww_tpdt, aidelta, ajdelta, xi, xj
...
sphere_sphere_contact: DO j=i+1,system%no_particles
  ...
  IF (overlap>=0.0_real8 .AND. normal_relative_vel>0.0_real8) THEN
    CALL resolve_sphere_sphere_contact(i,j,overlap,norm_ri_m_rj,xi,xj,ti,tj,vi,vj,wi_tpdt,wj_tpdt,aidelta,ajdelta,nij,system,htcond,a_1,l_1,E_i,E_j,h_i_j)
  END IF
...
sphere_wall_contact: DO k=1,system%no_rigid_walls
  ...
  IF (overlap>=0.0_real8 .AND. normal_relative_vel>0.0_real8 ) THEN
    CALL resolve_sphere_wall_contact(i,k,overlap,overlap_w,a_w,ti,tk,diw,vi,vw,wi_tpdt,ww_tpdt,ri,aidelta,n_out,system)
  END IF
...
SUBROUTINE detect_and_resolve_sphere_contacts_by_looping_over_all_part(i,R,V,A,W,system)
...
REAL(KIND=real8) :: norm_ri_m_rj, overlap, vin, vjn, normal_relative_vel, diw, vwn, time, &
                    xi_wall, xf_wall, yi_wall, yf_wall, zi_wall, zf_wall
REAL(KIND=real8), DIMENSION(3) :: ri, rj, ri_m_rj, nij, vi, vj, wi, wj, r1, ri1, n_out, vw, ww, aidelta, ajdelta, xciw, xi, xj
...
sphere_sphere_contacts: DO j=i+1,system%no_particles
  ...
  IF (overlap>0.0_real8) THEN
    ...
    END IF
    CALL resolve_sphere_sphere_contact(i,j,overlap,norm_ri_m_rj,xi,xj,ti,tj,vi,vj,wi,wj,aidelta,ajdelta,nij,system,htcond,a_1,l_1,E_i,E_j,h_i_j)
    !IF (system%general_cv%adhesion_switch=="on") THEN
      !CALL compute_sphere_sphere_adhesion_force_and_moment(i,j,overlap,nij,vi,vj,wi,wj,system,fadh)
      !!aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
      !IF (ALL(system%particle(1)%adhesion_force==0.0_real8)) THEN
      !  system%particle(1)%contact_force=0.0_real8
      !  system%particle(2)%contact_force=0.0_real8
      !END IF
      !!aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
    !END IF 
  END IF
...
sphere_wall_contacts: DO k=1,system%no_rigid_walls
  ...
  ri1 = r1 - ri
  diw = DOT_PRODUCT(ri1,n_out)
  !xxxxxxxxxxxxxxxx15/08I: comment part below if wall limits are to be enforced further down xxxxxxxxxxxxxxxxxxxxxxx
  IF (diw<0.0_real8) THEN
    WRITE(error_unit,*) " CAUTION: negative distance between particle", i, " and wall", k, " !"
  ...
  !xxxxxxxxxxxxx15/08Fxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
  diw = ABS(diw)   ! CAUTION: check whether this may spoil something...
  ...
  !xxxxxxxxxxxxxx15/08Ixxxxxxxxxxxxxxxxxxx define wall limits and check if contact point falls within xxxxxxxxxxxxxxxxxxxxxxxxxxxxx
  ...
  !  !END IF     
  !END IF
  !xxxxxxx15/08Fxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
  ...
  IF (overlap>0.0_real8) THEN
    ...
    IF (ALL(system%particle(i)%contacting_walls(1:ncw)%wall_number/=k)) THEN
      CALL add_wall_to_particle_contact_list(i,k,system)
    END IF
    CALL resolve_sphere_wall_contact(i,k,overlap,overlap_w,a_w,ti,tk,diw,vi,vw,wi,ww,ri,aidelta,n_out,system)
    !IF (system%general_cv%adhesion_switch=="on") THEN
    !  CALL compute_sphere_wall_adhesion_force_and_moment(i,k,overlap,n_out,vi,vw,wi,ww,system)
    !END IF     
  END IF
...
SUBROUTINE detect_and_resolve_sphere_contacts_by_verlet_lists(i,R,V,Tt,Tw,A,W,system,htcond,overlap_1,overlap_w,a_w,a_1,l_1,E_i,E_j,h_i_j)

!VARIABLES AGREGADAS: T, ti, tj
...
REAL(KIND=real8)   :: norm_ri_m_rj, overlap, vin, vjn, normal_relative_vel, diw, vwn, time, &
                    xi_wall, xf_wall, yi_wall, yf_wall, zi_wall, zf_wall, ti, tj, tk 
...
sphere_sphere_contacts: DO iv=1,ncn
  ...
  IF (j>i) THEN
    ...
    norm_ri_m_rj = SQRT(DOT_PRODUCT(ri_m_rj,ri_m_rj))
    overlap = system%particle(i)%radius + system%particle(j)%radius - norm_ri_m_rj 
	overlap_1 = system%particle(i)%radius + system%particle(j)%radius - norm_ri_m_rj !overlap_aqui
    ! compute normal direction for pair i-j
    nij = -(1.0_real8/norm_ri_m_rj)*ri_m_rj
    ...
    ! compute contact forces for pair i-j
    IF (overlap>0.0_real8) THEN !cuando hay contacto, ésta condición se cumple (delta_ij es mayor a cero)
      ...
      IF (ALL(system%particle(i)%contacting_particles(1:ncp)%part_number/=j)) THEN
        CALL add_particle_to_particle_contact_list(i,j,system)
      END IF
      CALL resolve_sphere_sphere_contact(i,j,overlap,norm_ri_m_rj,ri,rj,ti,tj,vi,vj,wi,wj,aidelta,ajdelta,nij,system,htcond,a_1,l_1,E_i,E_j,h_i_j)
      !IF (system%general_cv%adhesion_switch=="on") THEN
      !  CALL compute_sphere_sphere_adhesion_force_and_moment(i,j,overlap,nij,vi,vj,wi,wj,system)
      !END IF 
...
! check for contacts with walls and, if occurring, compute contact forces xxxxxxxxxxxxxxxxxxxxxxxxxxx
sphere_wall_contacts: DO k=1,system%no_rigid_walls
  ...
  n_out = system%wall(k)%outside_normal
  ri1 = r1 - ri
  diw = DOT_PRODUCT(ri1,n_out) ! distance between particle and wall <<<
  !xxxxxxxxxxxxxx 15/08I: comment part below if wall limits are to be enforced further down xxxxxxxxxxxxxxxxxxxxxxxxxxx
  IF (diw<0.0_real8) THEN
    WRITE(error_unit,*) " CAUTION: negative distance between particle", i, " and wall", k, " !"
     !WRITE(error_unit,*) " CAUTION: center of particle", i, " is beyond wall", k, "; check if this is ok"
  END IF
  !Think: maybe the warning above is not needed: only the warning for excessive overlap (further down) is needed...
  !xxxxxxxxxxxxx15/08Fxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
  diw = ABS(diw)   ! CAUTION: check whether this may spoil something...
  ...
  IF (overlap>0.0_real8) THEN
    ...
    END IF
    CALL resolve_sphere_wall_contact(i,k,overlap,overlap_w,a_w,ti,tk,diw,vi,vw,wi,ww,ri,aidelta,n_out,system) !aqui
    !IF (system%general_cv%adhesion_switch=="on") THEN
    !  CALL compute_sphere_wall_adhesion_force_and_moment(i,k,overlap,n_out,vi,vw,wi,ww,system)
    !END IF     
  END IF
...
SUBROUTINE detect_and_resolve_sphere_contacts_by_binning(i,R,V,A,W,system)
...
REAL(KIND=real8)   :: norm_ri_m_rj, overlap, vin, vjn, normal_relative_vel, diw, vwn, time
REAL(KIND=real8), DIMENSION(3) :: ri, rj, ri_m_rj, nij, vi, vj, wi, wj, r1, ri1, n_out, vw, ww, aidelta, ajdelta, xi, xj
...
SUBROUTINE detect_and_resolve_particle_contacts(i,R,V,Tt,Tw,A,W,sys,htcond,overlap_1,overlap_w,a_w,a_1,l_1,E_i,E_j,h_i_j)
...
SELECT CASE (sys%solution_cv%particles_shape)
  CASE ("spherical")
    CALL contact_solver_for_spheres(i,R,V,Tt,Tw,A,W,sys,htcond,overlap_1,overlap_w,a_w,a_1,l_1,E_i,E_j,h_i_j)
...
DO j=4,6
  ...
  IF (system%particle(i)%translational_dof_codes(j)==1 .AND. system%particle(i)%translational_dof_codes(j-3)==0) THEN
    ...
    !WRITE(error_unit,*) " Warnging: velocity constraints were applied for particle = ", i, " dof = ", j
...
! local variables
INTEGER(KIND=int4) :: i, j, k, s, ss, no_infiltrated_particles, no_particles_that_crossed_midsection
...
! Print Particle's contact force
WRITE(extra_results_unit,*) "Particle's contact force:" 
WRITE(extra_results_unit,"(2X,A,T22,A,T37,A,T52,A)") "Particle", "fconx", "fcony", "fconz"
DO i=1,system%no_particles
  WRITE(extra_results_unit,"(3X,I3,7X,3ES15.4)") i, system%particle(i)%contact_force
END DO
WRITE(extra_results_unit,*)
...
SUBROUTINE print_system_thermomechanics_results(system,overlap_1,overlap_w,a_w,a_1,l_1,E_i,E_j,h_i_j,hticondt)

! dummy arguments
TYPE(system_data) :: system
REAL(KIND=real8)  :: overlap_1, a_1, l_1, E_i, E_j, overlap_w, a_w, h_i_j, hticondt
...
SELECT CASE (system%general_cv%results_file_format)
  ...
  CASE ("gid_format")
    CALL print_thermomechanics_results_in_gid_format(system,overlap_1,overlap_w,a_w,a_1,l_1,E_i,E_j,h_i_j,hticondt)
  CASE DEFAULT
    WRITE(error_unit,*) "results file format not recognized (please check caps: only small caps are allowed for the results file format type)"
    STOP
END SELECT
...
SUBROUTINE print_thermomechanics_results_in_gid_format(system,overlap_1,overlap_w,a_w,a_1,l_1,E_i,E_j,h_i_j,hticondt) !overlap_aqui
...
! print file header (only when printing results for the first time)
IF (time_real4==first_tprint_real4.AND.system%general_cv%temperature_dofs=="on") THEN 
  WRITE(GiD_results_unit,*) "GiD Post Results File 1.0", " | ", "Particles' temperature ON"
  WRITE(GiD_results_unit,*)
  WRITE(GiD_results_unit,*)
  ELSEIF (time_real4==first_tprint_real4.AND.system%general_cv%temperature_dofs=="off") THEN
  WRITE(GiD_results_unit,*) "GiD Post Results File 1.0", " | ", "Particles' temperature OFF"
  WRITE(GiD_results_unit,*)
  WRITE(GiD_results_unit,*)
END IF
...
!========INICIO COMENTARIO=========
!DO i=1,1
!  WRITE(GiD_results_unit,"(ES15.6)") system%particle(i)%velocity(1)
!END DO

!WRITE(GiD_results_unit,"(ES15.6)") a_w
!========INICIO COMENTARIO=========
...
IF (time_real4==first_tprint_real4) THEN 
  ...
  DO i=1,system%no_particles
    ...
    !WRITE(GiD_mesh_unit,"(I10,TR2,I10,TR3,ES15.6)") i, i, 10.0_real8*system%particle(i)%radius
	WRITE(GiD_mesh_unit,"(I10,TR2,I10,TR3,ES15.6)") i, i, system%particle(i)%radius
    !xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
  END DO
  WRITE(GiD_mesh_unit,*) " End Elements"
...
REAL(KIND=real8)   :: t, tpdt, tprint, dt, dt_min, dt_max, dtcol, dt_print, phi, phi2, mi, tol1, tol2, tolR, tolV, Rerror, Verror, htcond, overlap_1, a_1, l_1, E_i, E_j, overlap_w, a_w, h_i_j
...
TYPE(array_of_vectors), DIMENSION(:), POINTER :: Rt, Rtpdt, Vt, Vtpdt, Fnft, Fnftpdt, Fenvt, Fenvtpdt, Fcont, Fcontpdt, &
                                                 Rtpdt_old, Vtpdt_old, Rresidual, Vresidual, Rnormalizer, Vnormalizer, Tt, Tw
...
DO i=1,system%no_particles
  i_kind = system%particle(i)%parameters%kind
  CALL detect_and_resolve_particle_contacts(i,Rt,Vt,Tt=Tt,Tw=Tw,sys=system,htcond=htcond,overlap_1=overlap_1,overlap_w=overlap_w,a_w=a_w,a_1=a_1,l_1=l_1,E_i=E_i,E_j=E_j,h_i_j=h_i_j)  ! caution: if there are particles in contact at t=0, they must have nonzero relative velocity for the contact not to be missed!
  ...
  10 WRITE(default_output_unit,*)
     ...
     iterations: DO iter=1,max_niter
       ...
       DO i=1,system%no_particles
         ...
         finft = Fnft(i)%r
         fienvt = Fenvt(i)%r
         CALL detect_and_resolve_particle_contacts(i,Rt,Vt,Tt=Tt,Tw=Tw,sys=system,htcond=htcond,overlap_1=overlap_1,overlap_w=overlap_w,a_w=a_w,a_1=a_1,l_1=l_1,E_i=E_i,E_j=E_j,h_i_j=h_i_j)
...
REAL(KIND=real8)   :: t, tpdt, tprint, tcellupdate, tverletupdate, dt, dt_min, dt_max, dt_print, dt_cell_list_update, &
                      dt_verlet_list_update, phi, phi2, mi, tol1, tol2, tolR, tolV, Rerror, Verror, htcond, overlap_1, a_1, l_1, E_i, E_j, overlap_w, a_w, h_i_j
...
TYPE(array_of_vectors), DIMENSION(:), POINTER :: Rt, Rtpdt, Vt, Vtpdt, Fnft, Fnftpdt, Fenvt, Fenvtpdt, Fcont, Fcontpdt, Ffrict, &
                                                 Ffrictpdt, Rtpdt_old, Vtpdt_old, Rresidual, Vresidual, Rnormalizer, Vnormalizer, Tt, Tw
  ...
  10 WRITE(default_output_unit,*)
     ...
     iterations: DO iter=1,max_niter
       ...
       DO i=1,system%no_particles
         ...
         ficont = Fcont(i)%r
         fifrit = Ffrict(i)%r
         CALL detect_and_resolve_particle_contacts(i,Rtpdt,Vtpdt,Tt=Tt,Tw=Tw,sys=system,htcond=htcond,overlap_1=overlap_1,overlap_w=overlap_w,a_w=a_w,a_1=a_1,l_1=l_1,E_i=E_i,E_j=E_j,h_i_j=h_i_j)
...
REAL(KIND=real8)   :: t, tpdt, tprint, tcellupdate, tverletupdate, dt, dt_print, dt_cell_list_update, dt_verlet_list_update, & 
                      phi, phi2, mi, Force_norm, Ref_norm, xmin, xmax, ymin, ymax, zmin, zmax, xi, yi, zi, htcond, overlap_1, a_1, l_1, E_i, E_j, overlap_w, a_w, h_i_j
REAL(KIND=real4)   :: tpdt_real4, tfinal_real4, tprint_real4, tcellupdate_real4, tverletupdate_real4
...
TYPE(array_of_vectors), DIMENSION(:), POINTER :: Rt, Rtpdt, Vt, Vtpdt, Fnft, Fenvt, Fcont, Ffrict, Tt, Tw
REAL(KIND=real8), PARAMETER    :: one = 1.0_real8   
...
REAL(KIND=real8)   :: t, tpdt, tprint, dt, dt_min, dt_max, dtcol, dt_print, phi, phi2, mi, ji, &
                      tol1, tol2, tolR, tolV, tolA, tolW, Rerror, Verror, Aerror, Werror, htcond, overlap_1, a_1, l_1, E_i, E_j, overlap_w, a_w, h_i_j
REAL(KIND=real4)   :: tpdt_real4, tfinal_real4, tprint_real4
...
TYPE(array_of_vectors), DIMENSION(:), POINTER :: Rt, Rtpdt, Vt, Vtpdt, At, Adelta, Wt, Wtpdt, Fnft, Fnftpdt, Fenvt, Fenvtpdt, &
                                                 ...
                                                 Rnormalizer, Vnormalizer, Anormalizer, Wnormalizer, Tt, Tw
...
DO i=1,system%no_particles
  i_kind = system%particle(i)%parameters%kind
  CALL detect_and_resolve_particle_contacts(i,Rt,Vt,Tt,Tw,Adelta,Wt,system,htcond,overlap_1,overlap_w,a_w,a_1,l_1,E_i,E_j,h_i_j)  ! caution: if there are particles in contact at t=0, they must have nonzero relative velocity for the contact not to be missed!
  CALL compute_particle_nearfield_forces(i,i_kind,Rt,Vt,system,finft)
  CALL compute_particle_environment_forces(i,i_kind,Rt,Vt,system,fienvt)
  system%particle(i)%nearfield_force = finft
  system%particle(i)%environment_force = fienvt
  ...
  10 WRITE(default_output_unit,*)
     ...
     iterations: DO iter=1,max_niter
       ...
       DO i=1,system%no_particles
         ...
         finft = Fnft(i)%r
         fienvt = Fenvt(i)%r
         CALL detect_and_resolve_particle_contacts(i,Rt,Vt,Tt,Tw,Adelta,Wtpdt,system,htcond,overlap_1,overlap_w,a_w,a_1,l_1,E_i,E_j,h_i_j)
         ficont = system%particle(i)%contact_force
         fifrit = system%particle(i)%friction_force
...
REAL(KIND=real8)   :: t, tpdt, tprint, tcellupdate, tverletupdate, dt, dt_print, dt_cell_list_update, dt_verlet_list_update, mi, ji, Ref_norm, htcond, overlap_1, a_1, l_1, E_i, E_j, overlap_w, a_w, h_i_j
...
TYPE(array_of_vectors), DIMENSION(:), POINTER :: Rt, Rtpdt, Vt, Vtpdt, At, Adelta, Wt, Wtpdt, Fnft, Fenvt, Fcont, &
                                                 Ffrict, Fadht, Mfrict, Mrolt, Madht, Tt, Tw
TYPE(contact_history_array), DIMENSION(:), ALLOCATABLE :: PPCHt, PWCHt
...
steps: DO s=1,system%solution_cv%no_steps
  ...
  substeps: DO WHILE (tpdt_real4<=tfinal_real4)
     ...
     DO i=1,system%no_particles
       CALL detect_and_resolve_particle_contacts(i,Rt,Vt,Tt,Tw,Adelta,Wt,system,htcond,overlap_1,overlap_w,a_w,a_1,l_1,E_i,E_j,h_i_j)
...
REAL(KIND=real8)   :: t, tpdt, tprint, tcellupdate, tverletupdate, dt, dt_min, dt_max, dt_print, dt_cell_list_update, &
                      dt_verlet_list_update, phi, phi2, mi, ji, tol1, tol2, tolR, tolV, tolA, tolW, Rerror, Verror, Aerror, Werror, htcond, overlap_1, a_1, l_1, E_i, E_j, overlap_w, a_w, h_i_j
REAL(KIND=real4)   :: tpdt_real4, tfinal_real4, tprint_real4, tcellupdate_real4, tverletupdate_real4
    ...
    END DO
     ...
     iterations: DO iter=1,max_niter
        ...
        DO i=1,system%no_particles
          ...
          !IF (s==1 .AND. ss==1) figivt = system%particle(i)%initial_given_force
          !IF (s==1 .AND. ss==1) migivt = system%particle(i)%initial_given_moment
          CALL detect_and_resolve_particle_contacts(i,Rtpdt,Vtpdt,Tt,Tw,Adelta,Wtpdt,system,htcond,overlap_1,overlap_w,a_w,a_1,l_1,E_i,E_j,h_i_j) 
          ficontpdt = system%particle(i)%contact_force
          fifritpdt = system%particle(i)%friction_force
...
! VARIABLES AGREGADAS: hticondt, HTcondt <<============================================================================LOOK HERE!

! local variables
CHARACTER(50)      :: i_kind, wkind
INTEGER(KIND=int4) :: s, ss, i, j, w, derror_R, derror_V, derror_A, derror_W, derror_T, derror_F
REAL(KIND=real8)   :: t, tpdt, tprint, tcellupdate, tverletupdate, dt, dt_print, dt_cell_list_update, dt_verlet_list_update, &
                      mi, ji, ci, cw, tit, titpdt, htienvt, Ref_norm, mi_mod, hticondt, overlap_1, a_1, l_1, E_i, E_j, overlap_w, a_w, h_i_j
REAL(KIND=real4)   :: tpdt_real4, tfinal_real4, tprint_real4, tcellupdate_real4, tverletupdate_real4
REAL(KIND=real8), DIMENSION(3) :: rit, ritpdt, vit, vitpdt, ait, aidelta, wit, witpdt, ficont, fifrit, finft, &
                                  fienvt, mifrit, mirolt, figivt, migivt, fiadht, miadht, mienvt 
TYPE(array_of_vectors), DIMENSION(:), POINTER :: Rt, Rtpdt, Vt, Vtpdt, At, Adelta, Wt, Wtpdt, Tt, Tw, Ttpdt, &
                                                 Fnft, Fenvt, Fcont, Ffrict, Fadht, Mfrict, Mrolt, Madht, Menvt, HTenvt, HTcondt, HT_wcondt
TYPE(contact_history_array), DIMENSION(:), ALLOCATABLE :: PPCHt, PWCHt
REAL(KIND=real8), PARAMETER :: one = 1.0_real8, four = 4.0_real8, half = 0.5_real8
...
ALLOCATE(Mfrict(1:system%no_particles),Mrolt(1:system%no_particles))
ALLOCATE(Fadht(1:system%no_particles),Madht(1:system%no_particles))
ALLOCATE(Menvt(1:system%no_particles),HTenvt(1:system%no_particles),HTcondt(1:system%no_particles))
ALLOCATE(PPCHt(1:system%no_particles),PWCHt(1:system%no_particles))
ALLOCATE(Tw(1:system%no_rigid_walls),HT_wcondt(1:system%no_rigid_walls))
...
DO i=1,system%no_particles
  ...
  Adelta(i)%r = 0.0_real8
END DO
!system%wall(i)%initial_temperature 
! get rigid walls´ initial conditions
DO w=1,system%no_rigid_walls
  system%wall(w)%point_position = system%wall(w)%initial_point_position
  system%wall(w)%velocity = system%wall(w)%initial_velocity
  ...
  Tw(w)%r = system%wall(w)%temperature
END DO
...
steps: DO s=1,system%solution_cv%no_steps
  ...
  substeps: DO WHILE (tpdt_real4<=tfinal_real4)
     ...
     DO i=1,system%no_particles
       ...
       system%particle(i)%rolling_resistance_moment = 0.0_real8
       system%particle(i)%adhesion_moment = 0.0_real8
	   system%particle(i)%conduction = 0.0_real8
     ...
     DO w=1,system%no_rigid_walls
       system%wall(w)%contact_force  = 0.0_real8
       system%wall(w)%friction_force = 0.0_real8
	   system%wall(w)%conduction     = 0.0_real8
     END DO
     
     ! compute particles´ contact, friction and adhesion force vectors and arrays at time t	<<====|>|AQUI|<|====
     DO i=1,system%no_particles
       CALL detect_and_resolve_particle_contacts(i,Rt,Vt,Tt,Tw,Adelta,Wt,system,hticondt,overlap_1,overlap_w,a_w,a_1,l_1,E_i,E_j,h_i_j)
       ! notice: at this point, the walls´ contact lists and total contact forces have been fully computed!
       Fcont(i)%r = system%particle(i)%contact_force
       Ffrict(i)%r = system%particle(i)%friction_force
       Fadht(i)%r = system%particle(i)%adhesion_force
       ...
       Mrolt(i)%r = system%particle(i)%rolling_resistance_moment
       Madht(i)%r = system%particle(i)%adhesion_moment
	   HTcondt(i)%r = system%particle(i)%conduction
	   !HT_wcondt(i)%r = system%wall(i)%conduction
	   !HTcondt(i)%r(1) = hticondt
	   !HTcondt(i)%r(1) = hticondt			!CONDUCTION<<========================================================
       CALL update_particle_contact_history_arrays(i,system,PPCHt,PWCHt)
     END DO
     ! compute particles nearfield and environment force vectors, heat transfer and arrays at time t
     DO i=1,system%no_particles
       i_kind = system%particle(i)%parameters%kind
       CALL compute_particle_nearfield_forces(i,i_kind,Rt,Vt,system,finft)
       CALL compute_particle_environment_forces(i,i_kind,Rt,Vt,system,fienvt,mienvt)
       CALL compute_particle_environment_heat_transfer(i,i_kind,Rt,Vt,Tt,system,htienvt)
       Fnft(i)%r  = finft
       Fenvt(i)%r = fienvt
       Menvt(i)%r = mienvt
       HTenvt(i)%r(1) = htienvt
     END DO
     ...
     ! print substep information on screen
	 !IF (system%general_cv%temperature_wall_dofs=="on") THEN
		!WRITE(default_output_unit,*)
		!WRITE(default_output_unit,*)
		!WRITE(default_output_unit,"(A,I3,A,A,I2,TR6,A,I9,TR6,A,ES13.6)") "NP =", system%no_particles, " | wall on | ", "step =", s, "substep =", ss, "t+dt =", tpdt
	 !END IF
	 
	 !WRITE(default_output_unit,"(A,A,I2,TR6,A,I9,TR6,A,ES13.6)") " step =", s, " substep =", ss, " t+dt =", tpdt
	 
	 !IF (system%general_cv%temperature_wall_dofs=="off") THEN
		!WRITE(default_output_unit,*)
		!WRITE(default_output_unit,*)
		!WRITE(default_output_unit,"(A,I3,A,A,I2,TR6,A,I9,TR6,A,ES13.6)") "NP =", system%no_particles, " | wall off | ", "step =", s, "substep =", ss, "t+dt =", tpdt
	 !END IF
	 
	  !provisorio = overlap
	 
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
IF (tpdt>=0.4_real8) THEN
  CONTINUE
END IF

IF (tpdt>=0.69_real8) THEN
  CONTINUE
END IF
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    ...
    DO i=1,system%no_particles
      ...
      miadht = Madht(i)%r
      mienvt = Menvt(i)%r
      htienvt = HTenvt(i)%r(1)
	  hticondt = HTcondt(i)%r(1) 
      figivt = 0.0_real8
      migivt = 0.0_real8
      !IF (s==1 .AND. ss==1) figivt = system%particle(i)%initial_given_force
      !IF (s==1 .AND. ss==1) migivt = system%particle(i)%initial_given_moment
      vitpdt = vit + (dt/mi)*(finft+fienvt+figivt) + (dt/mi)*(ficont+fifrit+fiadht) ! CALCULA LAS NUEVAS LAS VELOCIDADES
	  
	  !vitpdt = vit + (dt/mi)*(ficont+fienvt+fifrit) ! para validación con 2P
      ...
      ritpdt = rit + dt*vitpdt	!CALCULA LAS NUEVAS POSICIONES DE LAS PARTÍCULAS
	  
      IF (ANY(system%particle(i)%translational_dof_codes(1:3)/=0)) CALL enforce_position_constraints(i,system,ritpdt,vitpdt)
      
	  witpdt = wit + (dt/ji)*(mifrit+mirolt+miadht+mienvt+migivt)
	  !witpdt = wit + (dt/ji)*(mifrit) ! para validación con 2P
      ...
      !titpdt = tit + (dt/(mi*ci))*htienvt	! WITH CONVECTION
	  
	  !titpdt = tit + (dt/(mi*ci))*hticondt	! WITH CONDUCTION
	  
	  titpdt = tit + (dt/(mi*ci))*(hticondt+htienvt) ! WITH CONVECTION & CONDUCTION
      ...
      Ttpdt(i)%r(1) = titpdt
    END DO
	!cw = system%wall(i)%specific_heat   
    ! check for numerical instability (not a rigorous check)
	!V_norm = norm(Vtpdt)
	!Ref_norm = norm(Vt)
	!IF (Ref_norm<=1.0E-10) Ref_norm = 1.0_real8
    ...
    IF (tpdt_real4==tprint_real4) THEN 
      IF (system%general_cv%compute_system_properties=="yes") CALL compute_system_properties(system)
      CALL print_system_thermomechanics_results(system,overlap_1,overlap_w,a_w,a_1,l_1,E_i,E_j,h_i_j,hticondt)
      tprint = tprint + dt_print
      tprint_real4 = REAL(tprint,real4)
    END IF
    ...
    IF (system%solution_cv%contact_detection_algorithm=="verlet_list") THEN
      IF (tpdt_real4==tverletupdate_real4) THEN
		  !WRITE(default_output_unit,*)
		  !WRITE(default_output_unit,*)
		  !WRITE(default_output_unit,*) "NOW IS UPDATING VERLET LIST..."
		  !WRITE(default_output_unit,*)
		  !WRITE(default_output_unit,*)
		  !PAUSE
        CALL update_grid_cells_lists(system)
...
DEALLOCATE(Fnft,Fenvt,Fcont,Ffrict,Fadht,Mfrict,Mrolt,Madht,Menvt,HTenvt,HTcondt,STAT=derror_F)
CALL deallocate_contact_history_arrays(system%no_particles,PPCHt,PWCHt)
...
!-------------------------------------------------------------------------------------------------------
! temperature_wall_dofs switch (on/off)
CALL capture_keyword("$temperature_side_one_wall_dofs")
READ(input_file_unit,*) system%general_cv%temperature_side_one_wall_dofs
WRITE(error_unit,*) "CAUTION: make sure input data is compatible with the temperature dofs switch"

CALL capture_keyword("$temperature_side_two_wall_dofs")
READ(input_file_unit,*) system%general_cv%temperature_side_two_wall_dofs
WRITE(error_unit,*) "CAUTION: make sure input data is compatible with the temperature dofs switch"

CALL capture_keyword("$temperature_side_three_wall_dofs")
READ(input_file_unit,*) system%general_cv%temperature_side_three_wall_dofs
WRITE(error_unit,*) "CAUTION: make sure input data is compatible with the temperature dofs switch"

CALL capture_keyword("$temperature_side_four_wall_dofs")
READ(input_file_unit,*) system%general_cv%temperature_side_four_wall_dofs
WRITE(error_unit,*) "CAUTION: make sure input data is compatible with the temperature dofs switch"

CALL capture_keyword("$temperature_side_five_wall_dofs")
READ(input_file_unit,*) system%general_cv%temperature_side_five_wall_dofs
WRITE(error_unit,*) "CAUTION: make sure input data is compatible with the temperature dofs switch"
!
!-------------------------------------------------------------------------------------------------------
...
CALL capture_keyword("$critical_temperature")
	READ(input_file_unit,*) system%critical_temperature

!------------------------------------------------------------------------------------------------------------------------------------------------
!WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS-WALLS
	
! read number of rigid walls
	CALL capture_keyword("$no_rigid_walls")
		READ(input_file_unit,*) system%no_rigid_walls

! read rigid walls properties |MECHANICAL AND THERMAL|
	IF (system%no_rigid_walls>0) THEN
...
!CASO #1 --> | wall 1 OFF | wall 2 OFF | wall 3 OFF| wall 4 OFF | wall 5 OFF
	IF (system%general_cv%temperature_side_one_wall_dofs=="off".and.system%general_cv%temperature_side_two_wall_dofs=="off".and.system%general_cv%temperature_side_three_wall_dofs=="off".and.system%general_cv%temperature_side_four_wall_dofs=="off".and.system%general_cv%temperature_side_five_wall_dofs=="off") THEN
		READ(input_file_unit,*)
		READ(input_file_unit,*)
		READ(input_file_unit,*)
		READ(input_file_unit,*)
...
!WALL ONE----------------------------------------------------------------------------------------------------------------------------------------
	...
	!		     ||||||||[1]||||||||
	!					  ^		
!CASO #2 (exemplo_1) --> read wall one thermal properties | wall 1 ON | wall 2 OFF | wall 3 OFF | wall 4 OFF | wall 5 OFF
		
	IF (system%general_cv%temperature_side_one_wall_dofs=="on".and.system%general_cv%temperature_side_two_wall_dofs=="off".and.system%general_cv%temperature_side_three_wall_dofs=="off".and.system%general_cv%temperature_side_four_wall_dofs=="off".and.system%general_cv%temperature_side_five_wall_dofs=="off") THEN
	CALL capture_keyword("$material_sets_wall_one_thermal_properties")
		DO i=1,1
		READ(input_file_unit,*) row, system%wall(i)%kind, system%wall(i)%specific_heat, system%wall(i)%thermal_conductivity	
	IF (system%wall(i)%specific_heat==0.0) THEN
		WRITE(error_unit,*) "ERROR: specific heat is zero in material set (WALLS)", i, "; program will be terminated"
		STOP
	END IF  
		END DO  
	END IF

!CASO #2.1 |exemplo_1| -->| wall 1 ON | wall 2 OFF | wall 3 OFF | wall 4 OFF | wall 5 OFF
...
!CASO #2.2 |exemplo_1| --> read wall initial temperatures | wall 1 ON | wall 2 OFF | wall 3 OFF | wall 4 OFF | wall 5 OFF
		
	IF (system%general_cv%temperature_side_one_wall_dofs=="on".and.system%general_cv%temperature_side_two_wall_dofs=="off".and.system%general_cv%temperature_side_three_wall_dofs=="off".and.system%general_cv%temperature_side_four_wall_dofs=="off".and.system%general_cv%temperature_side_five_wall_dofs=="off") THEN
	CALL capture_keyword("$wall_one_initial_temperatures")
		DO i=1,1 
		READ(input_file_unit,*) row, system%wall(i)%kind, system%wall(i)%initial_temperature 
		END DO
		READ(input_file_unit,*)
		READ(input_file_unit,*)
		READ(input_file_unit,*)
...
! CASO #3 --> read thermal properties | wall 1 ON | wall 2 ON | wall 3 OFF | wall 4 OFF | wall 5 OFF
		
	IF (system%general_cv%temperature_side_one_wall_dofs=="on".and.system%general_cv%temperature_side_two_wall_dofs=="on".and.system%general_cv%temperature_side_three_wall_dofs=="off".and.system%general_cv%temperature_side_four_wall_dofs=="off".and.system%general_cv%temperature_side_five_wall_dofs=="off") THEN
	CALL capture_keyword("$material_sets_wall_one_thermal_properties")
		DO i=1,1
		READ(input_file_unit,*) row, system%wall(i)%kind, system%wall(i)%specific_heat, system%wall(i)%thermal_conductivity	
		IF (system%wall(i)%specific_heat==0.0) THEN
		...
		END DO
		!<<<<<<<<<<<<<<<<<<
	CALL capture_keyword("$material_sets_wall_two_thermal_properties")
		DO i=2,2
		READ(input_file_unit,*) row, system%wall(i)%kind, system%wall(i)%specific_heat, system%wall(i)%thermal_conductivity	
		IF (system%wall(i)%specific_heat==0.0) THEN
		WRITE(error_unit,*) "ERROR: specific heat is zero in material set (WALLS)", i, "; program will be terminated"
...
!CASO #3.1 -->| wall 1 ON | wall 2 ON | wall 3 OFF | wall 4 OFF | wall 5 OFF
...
!CASO #3.2 --> read wall initial temperatures | wall 1 ON | wall 2 ON | wall 3 OFF | wall 4 OFF | wall 5 OFF
	...
	CALL capture_keyword("$wall_one_initial_temperatures")
		...
		END DO	
		!<<<<<<<<<<<<<<<<<<
	CALL capture_keyword("$wall_two_initial_temperatures")
		DO i=2,2 
		READ(input_file_unit,*) row, system%wall(i)%kind, system%wall(i)%initial_temperature 
		END DO	
		READ(input_file_unit,*)
...
! CASO #4 -->| wall 1 ON | wall 2 ON | wall 3 ON | wall 4 ON | wall 5 ON
		
	IF (system%general_cv%temperature_side_one_wall_dofs=="on".and.system%general_cv%temperature_side_two_wall_dofs=="on".and.system%general_cv%temperature_side_three_wall_dofs=="on".and.system%general_cv%temperature_side_four_wall_dofs=="on".and.system%general_cv%temperature_side_five_wall_dofs=="on") THEN
	CALL capture_keyword("$material_sets_wall_one_thermal_properties")
		DO i=1,1
		READ(input_file_unit,*) row, system%wall(i)%kind, system%wall(i)%specific_heat, system%wall(i)%thermal_conductivity	
		IF (system%wall(i)%specific_heat==0.0) THEN
	...
	CALL capture_keyword("$material_sets_wall_two_thermal_properties")
		...
		END DO
		!<<<<<<<<<<<<<<<<<<
	CALL capture_keyword("$material_sets_wall_three_thermal_properties")
		DO i=3,3
		READ(input_file_unit,*) row, system%wall(i)%kind, system%wall(i)%specific_heat, system%wall(i)%thermal_conductivity	
		IF (system%wall(i)%specific_heat==0.0) THEN
		WRITE(error_unit,*) "ERROR: specific heat is zero in material set (WALLS)", i, "; program will be terminated"
		...
		END DO 
		!<<<<<<<<<<<<<<<<<<
	CALL capture_keyword("$material_sets_wall_four_thermal_properties")
		DO i=4,4
		READ(input_file_unit,*) row, system%wall(i)%kind, system%wall(i)%specific_heat, system%wall(i)%thermal_conductivity	
		IF (system%wall(i)%specific_heat==0.0) THEN
		WRITE(error_unit,*) "ERROR: specific heat is zero in material set (WALLS)", i, "; program will be terminated"
		...
		END DO 
		!<<<<<<<<<<<<<<<<<<
	CALL capture_keyword("$material_sets_wall_five_thermal_properties")
		DO i=5,5
		READ(input_file_unit,*) row, system%wall(i)%kind, system%wall(i)%specific_heat, system%wall(i)%thermal_conductivity	
		IF (system%wall(i)%specific_heat==0.0) THEN
		WRITE(error_unit,*) "ERROR: specific heat is zero in material set (WALLS)", i, "; program will be terminated"
	...
	CALL capture_keyword("$wall_two_initial_temperatures")
		...
		READ(input_file_unit,*) row, system%wall(i)%kind, system%wall(i)%initial_temperature 
		END DO				
	CALL capture_keyword("$wall_three_initial_temperatures")
		DO i=3,3 
		READ(input_file_unit,*) row, system%wall(i)%kind, system%wall(i)%initial_temperature 
		END DO
	CALL capture_keyword("$wall_four_initial_temperatures")
		DO i=4,4 
		READ(input_file_unit,*) row, system%wall(i)%kind, system%wall(i)%initial_temperature 
		END DO
	CALL capture_keyword("$wall_five_initial_temperatures")
		DO i=5,5 
		READ(input_file_unit,*) row, system%wall(i)%kind, system%wall(i)%initial_temperature 
		END DO
	END IF
		
! CASO PARTICULAR #1 -->| wall 1 ON | wall 2 ON | wall 3 ON | wall 4 OFF | wall 5 OFF
		
	IF (system%general_cv%temperature_side_one_wall_dofs=="on".and.system%general_cv%temperature_side_two_wall_dofs=="on".and.system%general_cv%temperature_side_three_wall_dofs=="on".and.system%general_cv%temperature_side_four_wall_dofs=="off".and.system%general_cv%temperature_side_five_wall_dofs=="off") THEN
	CALL capture_keyword("$material_sets_wall_one_thermal_properties")
		DO i=1,1
		READ(input_file_unit,*) row, system%wall(i)%kind, system%wall(i)%specific_heat, system%wall(i)%thermal_conductivity	
		IF (system%wall(i)%specific_heat==0.0) THEN
...
!WALL TWO----------------------------------------------------------------------------------------------------------------------------------------
	...
	!		     |||||||||||||||||||
	!
!CASO #5 -->| wall 1 OFF | wall 2 ON | wall 3 OFF | wall 4 OFF | wall 5 OFF
	IF (system%general_cv%temperature_side_one_wall_dofs=="off".and.system%general_cv%temperature_side_two_wall_dofs=="on".and.system%general_cv%temperature_side_three_wall_dofs=="off".and.system%general_cv%temperature_side_four_wall_dofs=="off".and.system%general_cv%temperature_side_five_wall_dofs=="off") THEN
		READ(input_file_unit,*)
		READ(input_file_unit,*)
	END IF

!CASO #5.1 --> read wall thermal properties | wall 1 OFF | wall 2 ON | wall 3 OFF | wall 4 OFF | wall 5 OFF 
...
!CASO #5.2 -->| wall 1 OFF | wall 2 ON | wall 3 OFF | wall 4 OFF | wall 5 OFF
...
!CASO #5.3 --> read wall initial temperatures | wall 1 OFF | wall 2 ON | wall 3 OFF | wall 4 OFF | wall 5 OFF
...
!CASO #6 -->| wall 1 OFF | wall 2 ON | wall 3 ON | wall 4 OFF | wall 5 OFF [exemplo_2 FENG]
		
	IF (system%general_cv%temperature_side_one_wall_dofs=="off".and.system%general_cv%temperature_side_two_wall_dofs=="on".and.system%general_cv%temperature_side_three_wall_dofs=="on".and.system%general_cv%temperature_side_four_wall_dofs=="off".and.system%general_cv%temperature_side_five_wall_dofs=="off") THEN
		READ(input_file_unit,*)
		READ(input_file_unit,*)
	CALL capture_keyword("$material_sets_wall_two_thermal_properties")
		DO i=2,2 
...
!CASO #6.1 --> read wall initial temperatures | wall 1 OFF | wall 2 ON | wall 3 ON | wall 4 OFF | wall 5 OFF [exemplo_2 FENG]
...
!WALL THREE----------------------------------------------------------------------------------------------------------------------------------------
	...
	!		     |||||||||||||||||||
	!		
!CASO #7 -->| wall 1 OFF | wall 2 OFF | wall 3 ON | wall 4 OFF | wall 5 OFF
		
	IF (system%general_cv%temperature_side_one_wall_dofs=="off".and.system%general_cv%temperature_side_two_wall_dofs=="off".and.system%general_cv%temperature_side_three_wall_dofs=="on".and.system%general_cv%temperature_side_four_wall_dofs=="off".and.system%general_cv%temperature_side_five_wall_dofs=="off") THEN
		READ(input_file_unit,*)
		READ(input_file_unit,*)
		READ(input_file_unit,*)
		READ(input_file_unit,*)
...
!CASO #7.1 -->| wall 1 OFF | wall 2 OFF | wall 3 ON | wall 4 OFF | wall 5 OFF
...
!CASO #7.2 -->| wall 1 OFF | wall 2 OFF | wall 3 ON | wall 4 OFF | wall 5 OFF
...
!CASO #7.3 --> read wall three initial temperatures | wall 1 OFF | wall 2 OFF | wall 3 ON | wall 4 OFF | wall 5 OFF
...
IF (system%general_cv%temperature_dofs=="on".OR.system%general_cv%temperature_dofs=="off") THEN ! ATENCIÓN: por el momento queda de ésta forma
...
WRITE(error_unit,*) "ATTENTION: make sure the contact model is compatible with the adhesion forces switch"
...
! ask for input file
WRITE(default_output_unit,*)
WRITE(default_output_unit,*)" Enter name of input file (20 characters, no extension)"
...
WRITE(u,*) "                                    OSVALDO D. QUINTANA-RUIZ   "
WRITE(u,*)
